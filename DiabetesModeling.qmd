...
We will fit a clssification tree and test multiple complexity parameters to find the best one, based on the lowest logLoss.
```{r}
...
The best logLoss was found when our cp = 0.001, giving us a logLoss of roughly 0.35
...
```
As we see, our logistic regression model had the best accuracy, so we will continue with this model through the API portion, allowing the user to investigate the different predictors.